...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    switch ($type) {
        ...
        case "category":
            ...
            // Comprueba si existe la categoría antes de eliminarla
...
if ($_SERVER["REQUEST_METHOD"] === "GET") {
    ...
    switch ($type) {
        case "product":
            deleteItem($conn, $id, 'products', 'product_id');
            break;

        case "category":
            deleteItem($conn, $id, 'categories', 'category_id');
            break;

        case "order":
            deleteItem($conn, $id, 'orders', 'id');
            break;
...
// Función genérica para eliminar un producto, categoría o pedido
function deleteItem($conn, $id, $table, $foreignKey)
{
    // Solo puede realizar esta acción el administrador
    if (!isAdmin()) {
        http_response_code(403);
    ...
    // Comprueba si existe el elemento antes de eliminarlo
    ...
    if ($table != 'orders') {
        $statement = $conn->prepare("SELECT COUNT(*) AS count FROM $table WHERE $foreignKey = :id");
        $statement->execute([":id" => $id]);
        $result = $statement->fetch(PDO::FETCH_ASSOC);
        ...
        if ($itemCount > 0) {
            if ($table == 'products') {
                $_SESSION["flash"] = ["message" => "No se puede eliminar el producto porque tiene pedidos asociados.", "type" => "danger"];
            } elseif ($table == 'categories') {
                $_SESSION["flash"] = ["message" => "No se puede eliminar la categoría porque tiene productos asociados.", "type" => "danger"];
            }
    ...
    // No hay elementos asociados --> eliminar el elemento
